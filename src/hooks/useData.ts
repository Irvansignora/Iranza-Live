  ...
  useEffect(() => {
    ...
    async function load() {
      const [{ data: m }, { data: t }] = await Promise.all([
        supabase.from('session_metrics').select('*').eq('session_id', sessionId).order('recorded_at', { ascending: false }).limit(1).single(),
        supabase.from('viewer_timeline').select('*').eq('session_id', sessionId).order('timestamp').limit(100),
      ])
      if (m) setMetrics(m)
      if (t) setTimeline(t)
    }
